    ...
    public function handle(Request $request, $course_slug = null, $section_id = null)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'video' => ['required', 'mimes:mp4,avi,qt,mkv,flv,wmv,mov,m4v', 'max:50000000']
            ]);
            if ($validator->fails()) {
            ...
            if ($course_slug && $section_id) {
                if (($section = Section::find($section_id)) && ($course = Course::where('slug', $course_slug)->first())) {
                    ...
                    $video->order = ($order = Video::where('course_id', $course->id)
                        ->where('section_id', $section->id)
                        ->max('order')) !== null ? $order + 1 : 0;
                } else {
                    return response()->json([], 422);
                }
            } else {
                $video->order = ($order = Video::where('user_id', auth()->user()->id)->max('order')) !== null ? $order + 1 : 0;
            }
            $video->save();
            ...
            return response()->json(['Video upload error. Try again later.'], 422);
        }
    }
}
