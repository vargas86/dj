...
class StudentRoomClient {

    constructor(localMediaEl, remoteVideoEl, remoteAudioEl, audioDevice, videoDevice, mediasoupClient, socket, room_id, name, successCallback) {
        this.name = name
    ...
    initSockets() {
        this.socket.on('consumerClosed', function ({
            ...
            this.remoteVideoEl.innerHTML = '';
            this.remoteAudioEl.innerHTML = '';
        }.bind(this))
        ...
        this.socket.on('newProducers', async function (data) {
            ...
            for (let {
                ...
                if (!this.producers.has(producer_id) && isMain === true) {
                    if (!(sharedPeer && sharedPeer == this.socket.id && kind == 'audio')) {
    ...
    async produce(type, deviceId = null) {
        ...
        try {
            ...
            if (!audio) {
                ...
                // elem.style.width = '234px'
                // elem.style.height = '133px'
                // elem.autoplay = true
                // elem.className = "vid"
    ...
    async consume(producer_id) {
        ...
        this.getConsumeStream(producer_id).then(function ({
            ...
            if (kind === 'video') {
                ...
                elem.id = consumer.id
                elem.playsinline = false
                elem.style.width = '829px'
                elem.style.height = '476px'
                elem.autoplay = true
                elem.className = "vid"
...
export default StudentRoomClient